...
def swapPositions(grid, values):
  firstValue = grid[values[0][0]][values[0][1]]
  secondValue = grid[values[1][0]][values[1][1]]
  for i in range(9):
    for j in range(9):
      if grid[i][j] == firstValue:
        grid[i][j] = secondValue
      elif grid[i][j] == secondValue:
        grid[i][j] = firstValue
def swapHorizontalBar(grid, values):
  firstBar = grid[values[0]]

def rowSwap(grid, first, second):
  second_vals = grid[second]
  grid[first] = second_vals
  grid[sedond] = second_vals

def colSwap(grid, first, second):
  second_vals = []
  for i in range(9):
    second_vals.append(grid[i][second])
  for i in range(9):
    grid[i][second] = grid[i][first]
  for i in range(9):
    grid[i][first] = second_vals[i]

def hSwapVal(grid, first, second):
  print("Horizontal swap")
  second_vals = grid[second[0]][second[1]]
  grid[second[0]][second[1]] = grid[first[0]][first[1]]
  grid[first[0]][first[1]] = second_vals

def vSwapVal(grid, first, second):
  print("Vertical swap")
  second_vals = grid[second[0]][second[1]]
  grid[second[0]][second[1]] = grid[first[0]][first[1]]
  grid[first[0]][first[1]] = second_vals
